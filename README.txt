...
jar/exe/dmg package, or run "java -jar SND-client.jar" to start the
...
desktop icons to start the SND server.


Using the SND-client.zip package:

Unpack the SND-client.zip package and open a shell in the resulting SND
directory. Execute the run.sh or start.sh script if using Lisnd or BSD, run.bat
...
the snd_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.nxt for
configuration files, logs, and the snd_db database. Unlike run.sh, start.sh
...
where the SND UI should now be available.
...
to the SND installation directory, as the use of special characters may result
...
values. You do not need to delete the defaults from SND-default.properties, the
settings in SND.properties override those in SND-default.properties. This way,
when upgrading the software, you can safely overwrite SND-default.properties
with the updated file from the new package, while your customizations remain
safe in the SND.properties file.


How to contribute?

There are many ways to contribute to SND. Here are some examples:
...
The SND software is a client-server application. It consists of a java server
...
requests which other SND client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
snd_db directory. When upgrading, you should not delete the old snd_db
...
snd_db, except that it will take some extra time to download the blockchain
from scratch.

The default SND client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file SND.log, which gets
overwritten at restart. In case of an error, the SND.log file may contain
...
nxt.jar file using the enclosed jar.sh script, and then SND.jar should be
